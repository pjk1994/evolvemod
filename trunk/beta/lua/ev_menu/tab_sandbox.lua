...
local function updateTab()
	...
	if ( LocalPlayer():EV_IsSuperAdmin() ) then
		tab.block:SetPos( tab.block:GetWide(), 0 )
	else
		tab.block:SetPos( 0, 0 )
	end
end
...
local function buildTab()
	...
	tab.block = vgui.Create( "DFrame", tab.container )
	tab.block:SetDraggable( false )
	tab.block:SetTitle( "" )
	tab.block:ShowCloseButton( false )
	tab.block:SetPos( 0, 0 )
	tab.block:SetSize( tab.container:GetWide(), tab.container:GetTall() )
	tab.block.Paint = function()
		surface.SetDrawColor( 46, 46, 46, 255 )
		surface.DrawRect( 0, 0, tab.block:GetWide(), tab.block:GetTall() )
		
		draw.SimpleText( "You need to be a super administrator to access this tab!", "ScoreboardText", tab.block:GetWide() / 2, tab.block:GetTall() / 2 - 20, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER )
	end
	if ( LocalPlayer():EV_IsSuperAdmin() ) then tab.block:SetPos( tab.block:GetWide(), 0 ) end
end
